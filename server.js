...
app.post('/register', async (req, res) => {
  try {
    ...
    const user = new User({ name, email, password, mobile, area, role: userRole });
    await user.save();
...
app.post('/issues', authenticate, authorize(['citizen']), async (req, res) => {
  try {
    const { title, description, category, location, priority, photo } = req.body;
    const issue = new Issue({
      title, description, category, location, priority, photo,
...
app.get('/my-issues', authenticate, authorize(['citizen']), async (req, res) => {
  try {
...
app.put('/issues/approve/:id', authenticate, authorize(['monitor']), async (req, res) => {
  try {
...
app.put('/issues/status/:id', authenticate, authorize(['monitor']), async (req, res) => {
  try {
    const { status, assignedWorkerName, assignedWorkerPhone, resolutionPhoto } = req.body;
    let updateData = { status };
    if (assignedWorkerName !== undefined) updateData.assignedWorkerName = assignedWorkerName;
    if (assignedWorkerPhone !== undefined) updateData.assignedWorkerPhone = assignedWorkerPhone;
    if (resolutionPhoto !== undefined) updateData.resolutionPhoto = resolutionPhoto;
...
app.put('/issues/remarks/:id', authenticate, authorize(['monitor']), async (req, res) => {
  try {
...
const setupDefaultData = async () => {
    try {
        if (await User.countDocuments() === 0) {
            await User.create([
                { name: 'Alice Citizen', email: 'citizen@test.com', password: 'password', role: 'citizen' },
                { name: 'Bob Monitor', email: 'monitor@test.com', password: 'password', role: 'monitor' }
            ]);
            console.log("Default users created: citizen@test.com, monitor@test.com (password: 'password')");
        }
    } catch (e) {
