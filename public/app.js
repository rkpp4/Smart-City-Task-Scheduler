...
document.addEventListener('DOMContentLoaded', () => {
    // Auth Tabs
    document.getElementById('tab-login').addEventListener('click', (e) => showAuthTab(e, 'login-form'));
    document.getElementById('tab-register').addEventListener('click', (e) => showAuthTab(e, 'register-form'));
    ...
    // Photo Upload
    document.getElementById('issue-photo-upload').addEventListener('change', handlePhotoUpload);

    // Monitor Update Status Form
    document.getElementById('form-update-status').addEventListener('submit', handleUpdateStatus);
    document.getElementById('form-submit-issue').addEventListener('submit', handleCitizenSubmit);
...
function loadApp() {
    ...
    document.getElementById('user-role-badge').textContent = currentUser.role === 'citizen' ? 'Citizen' : 'Monitor';
...
function buildSidebar() {
    ...
    const links = [
        { id: 'dashboard', icon: 'fa-chart-pie', label: 'Dashboard Overview', roles: ['citizen', 'monitor'], handler: loadDashboardStats },
        { id: 'submit-issue', icon: 'fa-plus-circle', label: 'File a Report', roles: ['citizen'], handler: loadSubmitIssue },
        { id: 'my-issues', icon: 'fa-folder-open', label: 'My Tracking', roles: ['citizen'], handler: loadMyIssues },
        { id: 'all-issues', icon: 'fa-satellite-dish', label: 'Control Center', roles: ['monitor'], handler: loadAllIssues }
    ];
...
async function loadMyIssues() {
    ...
    try {
        ...
        tbody.innerHTML = '';
        if(issues.length === 0) tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color: var(--text-muted);">No reports found.</td></tr>';
        
        issues.forEach(i => {
            const stClass = i.status.replace(" ", "-");
            const photoBtn = i.photo ? `<button onclick="viewPhoto('${i.photo}')" class="btn-sm btn-photo"><i class="fas fa-camera"></i> Issue Photo</button>` : '';
            const resBtn = i.resolutionPhoto ? `<button onclick="viewPhoto('${i.resolutionPhoto}')" class="btn-sm btn-photo" style="background:#10b981"><i class="fas fa-check-circle"></i> Resolution Photo</button>` : '';
    ...
    } catch (e) { tbody.innerHTML = 'Error loading'; }
}
...
async function loadAllIssues() {
    ...
    try {
        ...
        tbody.innerHTML = '';
        if(issues.length === 0) tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; color: var(--text-muted);">No active system reports.</td></tr>';
        
        issues.forEach(i => {
            const stClass = i.status.replace(" ", "-");
            const citizenName = i.createdBy ? i.createdBy.name : 'Unknown';
            const photoBtn = i.photo ? `<button onclick="viewPhoto('${i.photo}')" class="btn-sm btn-photo"><i class="fas fa-image"></i> Photo</button>` : '';
...
function openStatusModal(id, currentStatus) {
    ...
    document.getElementById('update-status-select').value = currentStatus !== 'Submitted' ? currentStatus : 'Approved';
    ...
    const checkStatus = () => {
        ...
        if(workerField) workerField.style.display = (s === 'In Progress' || s === 'Assigned') ? 'block' : 'none';
        if(photoField) photoField.style.display = (s === 'Resolved') ? 'block' : 'none';
    };
...
async function handleUpdateStatus(e) {
    ...
    try {
        await fetch(`${apiBase}/issues/status/${issueId}`, {
...
async function loadDashboardStats() {
    ...
    container.innerHTML = '<p style="color:var(--text-muted)">Loading system matrices...</p>';
    ...
    const icons = {
        ...
        inProgress: { icon: 'fa-spinner fa-spin', color: 'rgba(245, 158, 11, 0.15)' },
        resolved: { icon: 'fa-check-circle', color: 'rgba(16, 185, 129, 0.15)' },
        closed: { icon: 'fa-lock', color: 'rgba(5, 150, 105, 0.15)' },
    };

    try {
        const res = await fetch(`${apiBase}/dashboard/stats`, { headers: authHeaders() });
